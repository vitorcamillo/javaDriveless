...
import io.github.selenium.javaDriverless.cdp.CDPCommandExecutor;
 ...
 * </p>
 */
public class Target implements CDPCommandExecutor {

    private static final Logger logger = JavaDriverlessLogger.getLogger(Target.class);
     ...
     * Sincronizado para evitar race conditions com operações assíncronas em andamento.
     */
    private synchronized void onLoaded() {
        globalThis.clear();
        documentElem = null;
    ...
    public CompletableFuture<Object> executeScript(String script, Object[] args, boolean awaitPromise, String serialization) {
        ...
        // FIX: Scripts que contêm 'return' ou 'arguments[' são estilo Selenium
        // e esperam estar dentro de um corpo de função. Runtime.evaluate trata
        // o código como top-level, onde 'return' é ilegal.
        // Solução: wrappá-los dentro de uma IIFE (Immediately Invoked Function Expression).
        String expression = script;
        // FIX BUG-09: Verificação mais precisa — só wrappear se 'return' ou 'arguments['
        // aparecem no nível top-level (não dentro de funções/arrows aninhadas)
        if (needsIifeWrapping(script)) {
            expression = "(function(){" + script + "})()";
        }

        cdpArgs.put("expression", expression);
    ...
    /**
     * Verifica se o script precisa ser envolvido em uma IIFE.
     * <p>
     * Retorna true se 'return' ou 'arguments[' aparecem no nível top-level
     * do script (fora de funções/arrows aninhadas).
     * </p>
     *
     * @param script código JavaScript
     * @return true se precisa de IIFE wrapping
     */
    private static boolean needsIifeWrapping(String script) {
        // Verificação rápida: se não contém nenhuma keyword relevante, não precisa
        if (!script.contains("return") && !script.contains("arguments[")) {
            return false;
        }

        // Contar profundidade de aninhamento de funções
        // Simplificação: contar { } e verificar se return/arguments aparecem no nível 0
        int depth = 0;
        ...
        for (int i = 0; i < len; i++) {
            ...
            // Pular strings entre aspas
            if (c == '\'' || c == '"' || c == '`') {
                char quote = c;
                i++;
                while (i < len && script.charAt(i) != quote) {
                    if (script.charAt(i) == '\\') i++; // escapar
                    i++;
                }
                continue;
            }
            ...
            if (c == '/' && i + 1 < len && script.charAt(i + 1) == '*') {
                i += 2;
                while (i + 1 < len && !(script.charAt(i) == '*' && script.charAt(i + 1) == '/')) i++;
                i++;
                continue;
            }
            ...
            // Verificar keywords no nível top-level (depth 0)
            if (depth == 0) {
                if (script.startsWith("return", i) &&
                    (i == 0 || !Character.isLetterOrDigit(script.charAt(i - 1))) &&
                    (i + 6 >= len || !Character.isLetterOrDigit(script.charAt(i + 6)))) {
                    return true;
                }
                if (script.startsWith("arguments[", i) &&
                    (i == 0 || !Character.isLetterOrDigit(script.charAt(i - 1)))) {
                    return true;
                }
            }
    ...
    public CompletableFuture<WebElement> findElement(String by, String value, Float timeout) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            long sleepMs = 10; // PERF-01: Exponential backoff starting at 10ms

            while (elem == null) {
                try {
                    WebElement parent = getDocumentElem().join();
                    // FIX BUG-06: Usar timeout curto no inner call — o loop externo já controla retry
                    elem = parent.findElement(by, value, 0.1f).join();
                } catch (Exception e) {
                ...
                if (elem == null) {
                    try {
                        ...
                        sleepMs = Math.min(sleepMs * 2, 200); // Backoff até 200ms
    ...
    public CompletableFuture<List<WebElement>> findElements(String by, String value, float timeout) {
        return CompletableFuture.supplyAsync(() -> {
            long startNanos = System.nanoTime();
            long sleepMs = 10; // PERF-01: Exponential backoff

            while (true) {
                try {
                    WebElement parent = getDocumentElem().join();
                    List<WebElement> result = parent.findElements(by, value).join();
                    // FIX BUG-07: Só retornar se a lista não estiver vazia
                    if (!result.isEmpty()) {
                        return result;
                    }
                } catch (Exception e) {
                ...
                double elapsed = (System.nanoTime() - startNanos) / 1_000_000_000.0;
                if (elapsed > timeout) {
                    // Timeout: retornar lista vazia em vez de exception
    ...
    public CompletableFuture<Void> getScreenshotAsFile(String filename) {
        if (!filename.toLowerCase().endsWith(".png")) {
            JavaDriverlessLogger.warn(logger,
                "Nome usado para screenshot não corresponde ao tipo de arquivo. Deve terminar com .png");
        }
    ...
    public CompletableFuture<Void> saveSnapshot(String filename) {
        if (!filename.endsWith(".mhtml")) {
            JavaDriverlessLogger.warn(logger,
                "Nome usado para snapshot não corresponde ao tipo de arquivo. Deve terminar com .mhtml");
        }
    ...
    public CompletableFuture<Void> sendKeys(String text) {
        ...
        try {
            ...
            for (char c : text.toCharArray()) {
                ...
                if (keyInfo == null) {
                    JavaDriverlessLogger.warn(logger, "Tecla não mapeada: {}", c);
                    continue;
                }
