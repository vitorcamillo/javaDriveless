...
public class WebElement {
    ...
    public CompletableFuture<Void> sendKeys(String keys) {
        ...
        return click(true, 0.3, 2.0, 20.0)
            .thenCompose(v -> {
                // Enviar teclas SEQUENCIALMENTE (não em paralelo!)
                // Cada tecla precisa ser processada antes da próxima
                CompletableFuture<Void> chain = CompletableFuture.completedFuture(null);
                
                for (char c : keys.toCharArray()) {
                    chain = chain.thenCompose(ignored -> sendSingleKey(c));
                }
    ...
    /**
     * Envia uma única tecla com eventos keyDown + char + keyUp.
     * O campo 'text' é OBRIGATÓRIO para que o caractere seja digitado no input.
     */
    private CompletableFuture<Void> sendSingleKey(char key) {
        String keyStr = String.valueOf(key);
        
        // keyDown com text (necessário para inserir o caractere)
        Map<String, Object> downArgs = new HashMap<>();
        downArgs.put("type", "keyDown");
        downArgs.put("key", keyStr);
        downArgs.put("text", keyStr);
        
        // char event (garante que o caractere é inserido)
        Map<String, Object> charArgs = new HashMap<>();
        charArgs.put("type", "char");
        charArgs.put("text", keyStr);
        ...
        return target.executeCdpCmd("Input.dispatchKeyEvent", downArgs, null)
            .thenCompose(v -> target.executeCdpCmd("Input.dispatchKeyEvent", charArgs, null))
            .thenCompose(v -> {
                // Delay pequeno entre teclas para simular digitação humana
                try { Thread.sleep(30 + (long)(Math.random() * 50)); } catch (InterruptedException ignored) {}
    ...
    public CompletableFuture<Void> clear() {
        ...
        String script = "obj.value = ''; obj.dispatchEvent(new Event('input', { bubbles: true }));";
        return executeScript(script, null, null, false, 5.0f)
            .thenApply(v -> null);
    }
    ...
    public CompletableFuture<Boolean> isEnabled() {
        ...
        String script = 
            "return !obj.disabled && obj.getAttribute('disabled') === null;";
    ...
    public CompletableFuture<String> getTagName() {
        ...
        return executeScript("return obj.tagName", null, null, false, 5.0f)
    ...
    public CompletableFuture<String> getAttribute(String name) {
        ...
        String script = String.format("obj.getAttribute('%s')", name);
        return executeScript(script, null, null, false, 5.0f)
    ...
    public CompletableFuture<Object> getProperty(String name) {
        ...
        String script = String.format("obj['%s']", name);
        return executeScript(script, null, null, false, 5.0f);
    }
    ...
    public CompletableFuture<double[]> getLocation() {
        ...
        String script = "var rect = obj.getBoundingClientRect(); " +
    ...
    public CompletableFuture<double[][]> getQuads() {
        ...
        return executeScript(script, null, null, false, 5.0f)
            .thenApply(result -> {
                if (result instanceof List) {
                    List<?> list = (List<?>) result;
                    double[][] quads = new double[4][2];
                    for (int i = 0; i < Math.min(4, list.size()); i++) {
                        if (list.get(i) instanceof List) {
                            List<?> point = (List<?>) list.get(i);
                            quads[i][0] = ((Number) point.get(0)).doubleValue();
                            quads[i][1] = ((Number) point.get(1)).doubleValue();
                        }
                    }
                    return quads;
                }
    ...
    private CompletableFuture<Object> executeScript(String script, Object arg, String serialization,
        ...
        return ensureObjId().thenCompose(v -> {
            ...
            cdpArgs.put("returnByValue", serialization == null);  // true para valores primitivos, false para WebElements
    ...
    public CompletableFuture<List<WebElement>> findElements(String by, String value) {
        ...
        if (By.ID.equals(by)) {
            ...
            // FIX BUG-08: Usar contains() com word boundaries para classes compostas
            // Exemplo: class="foo bar" deve ser encontrado por CLASS_NAME="foo"
            actualBy = By.XPATH;
            actualValue = String.format("//*[contains(concat(' ', normalize-space(@class), ' '), ' %s ')]", value);
    ...
    public boolean equals(Object obj) {
        ...
        // FIX BUG-11: Incluir backendNodeId na comparação para evitar
        // falsos positivos quando objId e nodeId são ambos null
        return Objects.equals(objId, other.objId) && 
               Objects.equals(nodeId, other.nodeId) &&
               Objects.equals(backendNodeId, other.backendNodeId);
    }
    
    @Override
    public int hashCode() {
        return Objects.hash(objId, nodeId, backendNodeId);
    }
}
