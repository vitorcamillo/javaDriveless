...
public class Chrome implements AutoCloseable {
    ...
    private String mv3Extension;
    private Boolean extensionsIncognitoAllowed;
    ...
    public CompletableFuture<Chrome> startSession() {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                // Adicionar extensão MV3 se necessário
                if (options.useExtension) {
                    try {
                        String extensionPath = Chrome.class.getClassLoader()
                            .getResource("mv3_extension").getPath();
                        options.addExtension(extensionPath);
                    } catch (Exception e) {
                        System.err.println("Aviso: Não foi possível carregar extensão MV3: " + e.getMessage());
                    }
                }
                ...
                // Processar extensões
                if (!options.getExtensionPaths().isEmpty()) {
                    List<String> extensionPaths = new ArrayList<>();
                    for (String extPath : options.getExtensionPaths()) {
                        // TODO: Extrair ZIPs se necessário
                        extensionPaths.add(extPath);
                    }
                    if (!extensionPaths.isEmpty()) {
                        options.addArgument("--load-extension=" + String.join(",", extensionPaths));
                    }
                }
                ...
                for (JsonNode targetNode : targets) {
                    ...
                    if ("page".equals(type) && !url.startsWith("chrome-extension://")) {
                ...
                if (options.getSingleProxy() != null) {
                    // TODO: Implementar setSingleProxy
                }
    ...
    /**
     * Obtém a extensão MV3 (Manifest V3) se carregada.
     *
     * @return caminho da extensão MV3 ou null
     */
    public String getMv3Extension() {
        return mv3Extension;
    }
     ...
     * Suporta formato: "http://user:pass@host:port/"
     *
     * AVISO: Requer extensão MV3 carregada
     *
     * @param proxy string do proxy (ex: "http://user:pass@example.com:8080/")
    ...
    public CompletableFuture<Void> setSingleProxy(String proxy, List<String> bypassList) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                // Build proxy config
                final List<String> finalBypassList = (bypassList == null) ? List.of("<local>") : bypassList;
                
                Map<String, Object> rules = new HashMap<>();
                rules.put("proxyForHttp", rule);
                rules.put("proxyForHttps", rule);
                rules.put("proxyForFtp", rule);
                rules.put("fallbackProxy", rule);
                rules.put("bypassList", finalBypassList);
                
                Map<String, Object> proxyConfig = new HashMap<>();
                proxyConfig.put("mode", "fixed_servers");
                proxyConfig.put("rules", rules);
                
                // Apply proxy
                setProxy(proxyConfig.toString(), finalBypassList).join();
                
                // Apply authentication if provided
                final String finalCreds = creds;
                final String finalHost = host;
                final Integer finalPort = port;
                if (finalCreds != null && finalCreds.contains(":")) {
                    String[] credParts = finalCreds.split(":", 2);
                    String user = credParts[0];
                    String pass = credParts[1];
                    String hostPort = finalHost + (finalPort != null ? ":" + finalPort : "");
                    
                    setAuth(user, pass, hostPort).join();
                }
    ...
    /**
     * Garante que extensões possam ser usadas em incognito.
     *
     * @return CompletableFuture que completa quando configurado
     */
    public CompletableFuture<Void> ensureExtensionsIncognitoAllowed() {
        if (extensionsIncognitoAllowed) {
            return CompletableFuture.completedFuture(null);
        }
        
        extensionsIncognitoAllowed = true;
        return CompletableFuture.completedFuture(null);
    }
