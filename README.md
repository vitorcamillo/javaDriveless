...
O JavaDriverless é silencioso por padrão para rodar bem em Docker/servidor. Use variáveis de ambiente para aumentar a visibilidade quando precisar diagnosticar:

```bash
# off, error, warn, info, debug ou trace. Padrao: warn
JAVA_DRIVERLESS_LOG_LEVEL=warn

# Mostra eventos de profile/PID. Padrao: false
JAVA_DRIVERLESS_LOG_PROFILE_EVENTS=false

# Mostra saida do processo Chrome/Chromium. Padrao: false
JAVA_DRIVERLESS_CHROME_LOGS=false

# Inclui o tail do Chrome nas excecoes de startup. Padrao: true
JAVA_DRIVERLESS_LOG_CHROME_TAIL_ON_ERROR=true

# Mostra banner/resumo de startup. Padrao: false
JAVA_DRIVERLESS_LOG_STARTUP_BANNER=false

# Timeout de startup do Chrome. Padrao: 45.0
JAVA_DRIVERLESS_STARTUP_TIMEOUT_SECONDS=45
```

Para investigar falhas de servidor sem poluir producao, normalmente basta:

```bash
JAVA_DRIVERLESS_LOG_LEVEL=debug
JAVA_DRIVERLESS_CHROME_LOGS=true
```
